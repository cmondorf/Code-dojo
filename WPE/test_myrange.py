...
def test_myrange2(first, second, third, output):
    if third:
        assert myrange.myrange2(first, second, third) == output
    else:
        assert myrange.myrange2(first, second) == output


def test_myrange3_is_a_generator():
    output = myrange.myrange3(10)
...
def test_myrange3(first, second, third, output):
    if third:
        assert list(myrange.myrange3(first, second, third)) == output
    else:
        assert list(myrange.myrange3(first, second)) == output
